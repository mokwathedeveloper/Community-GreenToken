...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      // Phase 2: supabase.auth.signUp({ email, password, options: { data: { display_name: name } } })
      await new Promise((r) => setTimeout(r, 800));
      window.location.href = inviteToken ? "/dashboard" : "/org/setup";
    } catch {
    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:block lg:w-1/2 relative">
        <Image
          ...
          alt="Diverse community group planting trees and nurturing green spaces together"
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center bg-white px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-gray-500 text-center mb-6">Join a global community building a sustainable and regenerative future.</p>
          ...
          {error && (
            <div role="alert" className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4 text-sm text-red-700 flex gap-2">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-3">
            ...
            <div>
              ...
              {password && (
                <div id="pwd-strength" aria-live="polite">
                  ...
                  <p className="text-xs text-gray-400 mt-1">{STRENGTH_LABELS[strength]} — Use 8+ characters with letters, numbers &amp; symbols.</p>
                </div>
              )}
            </div>
            ...
            {!inviteToken && (
              <Input id="signup-invite" type="text" label="Invite token (optional)"
                placeholder="Leave blank to create a new organization" hint="Have a team invite link? Paste the token here." />
            )}

            {/* Terms checkbox — R-A11Y-03 */}
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            {["Google", "GitHub"].map((provider) => (
              <button key={provider} type="button"
                className={cn("flex items-center justify-center gap-2 py-2.5 border border-gray-200 rounded-xl text-xs font-medium text-gray-700",
          ...
          <button type="button"
            className={cn("w-full flex items-center justify-center gap-2 py-2.5 border-2 border-primary-300",
              "text-sm font-semibold text-primary-700 rounded-xl hover:bg-primary-50 transition-colors",
              "focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:outline-none")}>
            🔗 Connect Crypto Wallet <span className="ml-1 text-xs bg-primary-100 text-primary-600 px-1.5 py-0.5 rounded">Web3</span>
          </button>
